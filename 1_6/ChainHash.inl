...
inline bool ChainHash<Key, Value, Func, BucketSize>::Add(const Key& inKey, const Value& inValue)
{
	...
	{	// 格納されていなかったら
		...
		// ハッシュ値を求める
		Hash tHash = Func(inKey) % BucketSize;
		// 対象チェインの先頭に追加
...
template <typename Key, typename Value, unsigned int (*Func)(Key), unsigned int BucketSize>
inline bool ChainHash<Key, Value, Func, BucketSize>::GetChainByHash(const unsigned int& inHash, LinkedList<Pair>& outList) const
{
	// バケットサイズに丸めたハッシュ値を再計算する
	unsigned int tHash = inHash % BucketSize;

	// 該当チェインに要素が存在しない場合も、即時終了
	if (_List[tHash].Size() == 0) { return false; }

	// リストをコピー
	if (!_List[tHash].Copy(outList))
	{	// コピーに失敗したら
...
template <typename Key, typename Value, unsigned int (*Func)(Key), unsigned int BucketSize>
inline bool ChainHash<Key, Value, Func, BucketSize>::GetAllData(LinkedList<Pair>& outList) const
{
	...
	{
		// 各チェインリストを結合していく
		if (!outList.Append(const_cast<LinkedList<Pair>&>(_List[i])))
		{	// 結合に失敗したら
