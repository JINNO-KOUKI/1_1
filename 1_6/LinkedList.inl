...
/// @brief			自身の情報をコピーする
/// @tparam T		格納されているデータ型
/// @param dstList	コピー先リスト
/// @return			正常終了 : TRUE  異常終了 : FALSE
/// @details		自身のリストのコピーを作成します。\n
///					コピー先に既に要素が存在する場合、何もせずに終了します。\n
///					※ この関数は、高速な動作を保証しません。\n
///					   コピーする要素数分のInsert()が実行されます。\n
template <typename T>
inline bool LinkedList<T>::Copy(LinkedList<T>& dstList)
{
	// コピー先に、既に格納済みの要素がある場合は終了
	if (dstList._Size > 0) { return false; }

	// -- 自身に格納されているすべての要素を走査
	for (LinkedList<T>::Iterator it = this->Begin(); it != this->End(); ++it)
	{
		// 一つずつdstに格納していく
		if (!dstList.Insert(dstList.End(), *it))
		{	// もし格納に失敗したら
...
/// @brief			自身の末尾に別のリストを結合する
/// @tparam T		格納されているデータ型
/// @param srcList	末尾に結合するリスト
/// @return			正常終了 : TRUE  異常終了 FALSE
/// @details		自身の末尾に、指定されたリストの要素を結合します。\n
///					※ この関数は、高速な動作を保証しません。\n
///					   結合する要素分のInsert()が実行されます。\n
///					相手のリストは削除されず、保持されます。\n
///					要素の格納に失敗した場合、Assertが発生します。
template <typename T>
inline bool LinkedList<T>::Append(LinkedList<T>& srcList)
{
	// -- コピー元に格納されているすべての要素を走査
	LinkedList<T>::Iterator tSrcEndIt = srcList.End();
	for (LinkedList<T>::Iterator it = srcList.Begin(); it != tSrcEndIt; ++it)
	{
		// 一つずつ自身に格納していく
		// 格納に失敗した場合、元データの保持ができないためAssertで終了する
		assert(this->Insert(this->End(), *it));
	}
