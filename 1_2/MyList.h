...
public:
	...
	class ConstIterator
	{
		...
		friend MyList;	// メンバ変数へのアクセスは、サブクラスとMyListクラスからのみ可能としています
	...
	public:	// コンストラクタ群
		...
		ConstIterator(Node* target, const MyList* list) : _Target(target), _List(list) {}
	...
	public:	// コンストラクタ群
		...
		Iterator(Node* target, MyList* list) : ConstIterator(target, list) {}
